...
//
//  rooms/{code}                — estado da sala (fase, zonas...)
...
export async function createRoom(prepSeconds: number, charId: string): Promise<string> {
  ...
  await joinRoomDoc(code, charId);
  return code;
}

export async function joinRoom(code: string, charId: string): Promise<string | null> {
  const normalized = code.trim().toUpperCase();
  const snap = await getDoc(doc(db, 'rooms', normalized));
  if (!snap.exists()) return null;
  if (snap.data().status !== 'waiting') return null;

  await joinRoomDoc(normalized, charId);
  return normalized;
}

async function joinRoomDoc(code: string, charId: string): Promise<void> {
  const uid = myUid();
...
export async function finishRoom(code: string, winnerUid: string, winnerName: string): Promise<void> {
  await updateDoc(doc(db, 'rooms', code), {
    status: 'finished',
    winnerUid,
    winnerName,
...
export function generateZones(): ZoneKeyframe[] {
  const W = CFG.WORLD;
  const HOLD = 18;     // segundos parado
  const SHRINK = 22;   // segundos encolhendo
  const STAGES = 5;
  ...
  for (let i = 0; i < STAGES; i++) {
    // Segura o círculo atual
    t += HOLD;
    zones.push({ x, y, r, t });

    // Próximo círculo: menor, centro aleatório dentro do atual e do mapa
    const nr = Math.max(150, r * 0.55);
    const angle = Math.random() * Math.PI * 2;
    const dist = Math.random() * Math.max(0, r - nr);
    const margin = 120;
    const nx = Math.min(W - nr - margin, Math.max(nr + margin, x + Math.cos(angle) * dist));
    const ny = Math.min(W - nr - margin, Math.max(nr + margin, y + Math.sin(angle) * dist));
  ...
  // Círculo final segura até o fim da partida
  t += 9999;
  zones.push({ x, y, r, t });
...
export function getZoneAt(
  ...
  if (zones.length === 0) {
    const c = { x: CFG.WORLD / 2, y: CFG.WORLD / 2, r: CFG.WORLD, t: 0 };
    return { current: c, target: null };
  }

  let i = 0;
  while (i < zones.length - 1 && zones[i + 1].t <= elapsedSec) i++;
  ...
  // Próximo alvo: primeiro keyframe à frente com geometria diferente
